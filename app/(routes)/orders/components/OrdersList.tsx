...
const OrdersList = (props: OrdersListProps) => {
  const { purchases, recepipts } = props

  const totalPurchases = purchases.reduce((acc, purchase) => {
    const rawPrice = purchase.course.price
    const price = rawPrice && !isNaN(Number(rawPrice)) ? rawPrice : 0
    return acc + price
  }, 0)

  const formattedTotal = formatPrice(totalPurchases)
  const downloadReceipt = (index: number) => {
    const receipt = recepipts?.[index]
